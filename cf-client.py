...
class CloudflareClient:
            ...
            if 'addresses' in host:
                ...
                for address in host['addresses']:
                    ...
                    if tmp.version == 4:
                        ...
                        ext_records.append(
                            DNSRecord(f"{name}.1.{self.domain}", RecordType.A, address, TTL_HOST))
                        ans.append(
                        ...
                        ext_records.append(
                            DNSRecord(f"{name}.1.{self.domain}", RecordType.AAAA, address, TTL_HOST))
                        ans.append(
                            DNSRecord(f"{name}.6.{self.domain}", RecordType.AAAA, address, TTL_HOST))
                ...
                for address in host['ext_addresses']:
                    ...
                    if tmp.version == 4:
                        ...
                        ans.append(
                            DNSRecord(f"{name}.1.{self.domain}", RecordType.AAAA, address, TTL_EXT))
                        ans.append(
                            DNSRecord(f"{name}.6.{self.domain}", RecordType.AAAA, address, TTL_EXT))
                ...
                for mid_name in host['mid_names']:
                    ...
                    if '-v' in basename:
                        ...
                        if mid_name.get('current', False):
                            ...
                            ans.append(DNSRecord(f"{network_name}.{self.domain}", RecordType.CNAME,
                                       f"{basename}{ZONE_SUFFIX[current_zone]}.{self.domain}", TTL_NET))
                    else:
                        current_zone = mid_name.get('current_zone', 0)
                        ans.append(DNSRecord(
                            f"{basename}.{self.domain}", RecordType.CNAME, f"{name}.{current_zone}.{self.domain}", TTL_NET))
        # validate
        names = [record.name for record in ans]
        for record in ans:
            assert record.name.endswith(self.domain)
            if record.type == RecordType.CNAME:
                assert len([i for i in names if i == record.name]
                           ) == 1, f"Error: {record.name} is not unique"
        return ans
    ...
    def update_dns(self, records):
        ...
        for index, record in enumerate(pending_records):
            for cf_record in cf_records:
                if record.sims(cf_record):
                    logging.debug(
                        f"Updating '{record.name}' from '{cf_record.content}' ({type(cf_record.content)}) to '{record.content}' ({type(record.content)})")
        ...
        for i in deleting_records:
            print(i)
        # adding records
        op_counts = len(adding_records) + len(updating_records) + \
            len(deleting_records)
        if op_counts == 0:
